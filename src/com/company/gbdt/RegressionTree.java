...
public class RegressionTree {
	...
	public Double Predict(Data.tuple datapoint) {
		return Predict(datapoint, root);
	}
	public Double Predict(Data.tuple datapoint, node rootnode) {
		if (rootnode.is_leaf) {
	...
	public void FitData(ArrayList<Data.tuple> sample, int sample_num) {
		 FitData(sample, sample_num,0,root);
	}
	
	public void FitData(ArrayList<Data.tuple> sample, int sample_num, int dep, node rootnode){
	
		if(configure.lossType == configure.loss_type.SQUARED_ERROR) {
	...
	public boolean FindandSplit(ArrayList<Data.tuple> sample, int sample_num, node rootnode) {
		...
		ArrayList<Data.tuple> useful_sample = new ArrayList<Data.tuple>();
	...
	public boolean getsplit(ArrayList<Data.tuple> sample, int sample_num, Integer fid, Returevalue thisReturevalue) {
		...
		for(Data.tuple datapoint : sample) {
